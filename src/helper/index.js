...
const { auth0, jwtoken, client_url } = require('../config/env');
...
class Helper {
  ...
  static queryHelper = (options) => {
    ...
    const { minPrice, maxPrice, productId } = query;
    ...
    // if product id, exculde the product from the list
    if (productId) {
      filtersData._id = {
        ...filtersData._id,
  ...
  static jwtHelper = {
     ...
     * @return {Data}
     * */
    verifyUserRefreshSecret: (token) =>
      jwt.verify(token, jwtoken.refresh_secret)
  };
}
